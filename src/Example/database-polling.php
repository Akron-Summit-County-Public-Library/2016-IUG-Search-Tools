...
$barcodes = file_get_contents('ID.csv');
$barcodes = explode("\n", $barcodes);

foreach ($barcodes as $barcode)
{
    $template['single_barcode'] = $barcode;

    $command = (new PaginatedQueryCommand)->setPage($page)->setLimit($limit);
    ...
    if (! $sql_file_name) {
        throw new \InvalidArgumentException(sprintf(
            'Usage: php %s "SQL File Name"'.PHP_EOL.PHP_EOL.'Please see Queries folder for available search types',
    ...
    //--- Return SQL results ---//
    $include_headers = ($result === true);
    $append = ($include_headers === false);
    ...
    //--- Render results ---//
    $json_view = new Json;
    $csv_view = new Csv($http_attachment = false, $include_headers, $append);
